...
5. Output to `client/public/videos/`— served at`/videos/` on the luxury-perfume domain.

##### FFmpeg in luxury-perfume NixOS
...
- Always resolve the domain with `echo $luxury-perfume_DOMAINS` before setting a canvas video shape URL. Never hardcode a domain — luxury-perfume domains change between sessions.
