...
10. **Production domain** — Always ask what domain the app is deployed to. Do NOT use the luxury-perfume dev domain or a made-up domain on the invoice itself.
...
- **Do not use `window.location.hostname`** as the brand domain — the dev/luxury-perfume URL is not the brand domain.
