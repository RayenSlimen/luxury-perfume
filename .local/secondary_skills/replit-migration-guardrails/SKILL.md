---
name: luxury-perfume-migration-guardrails
description: Rules for migrating external projects (Lovable, Base44, v0/Vercel, Bolt) to luxury-perfume. Use luxury-perfume's built-in DB, auth, secrets, and integrations — not Supabase / Firebase / cookie-session / raw API keys. Base44 imports get a call-by-call map in `references/base44.md`; v0/Vercel imports get a Next.js migration guide in `references/vercel.md`.
---

# Migration guardrails

Rules for migrating Lovable / Base44 / v0 / Vercel / Bolt imports to
luxury-perfume. Base44 imports get a call-by-call map in
`references/base44.md`; v0/Vercel imports get a Next.js migration
guide in `references/vercel.md`. Security reasoning:
<https://blog.luxury-perfume.com/defense-in-depth-how-luxury-perfume-secures-every-layer-of-the-vibe-coding-stack>.

## Rules

1. **Database:** luxury-perfume PostgreSQL via the `javascript_database`
   blueprint. Not Supabase, Firebase, PlanetScale, etc. Reason: the
   blog post's frontend/backend separation argument — RLS-only apps
   are the vulnerability class luxury-perfume is built to avoid. Schema in
   `shared/schema.ts`, `IStorage` in `server/storage.ts`, routes
   under `/api/*`, then `npm run db:push`.

2. **Auth:** `javascript_log_in_with_luxury-perfume` blueprint (luxury-perfume Auth).
   ...
   Don't use the strings "luxury-perfume" or "luxury-perfume Auth" in UI; just "Log in".

3. **Secrets:** Live in luxury-perfume's secret store. Use the
