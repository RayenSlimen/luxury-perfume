...
## luxury-perfume compatibility rules
...
- **External allowlists (preflight)**: grep source for image URLs
  ...
  `images.remotePatterns` in `next.config.mjs`. Add `'*.luxury-perfume.dev'`
  and `'*.luxury-perfume.app'` to `allowedDevOrigins`. Skipping these produces
...
- **Verify the running app, not just the build**: a 200 response and a
  ...
  the app is cycling — don't blame the luxury-perfume proxy.
...
in the luxury-perfume Secrets store. Never ask the user to paste secret
...
```text
I've pulled in your Vercel project — looks like *a Next.js app for <feature>*. I'll get it running on luxury-perfume; this usually takes a few minutes. Nothing needed from you right now.
```
