...
The user just imported a project from Vercel or v0. Port it from the imported state into the luxury-perfume `pnpm_workspace` stack so it runs cleanly in this environment. **It must look and work like the original.**
...
- Convert the Next.js app to Vite + React and restructure into the luxury-perfume `pnpm_workspace` layout (frontend artifact + optional `api-server` for API routes).
...
- **Env vars** — anything via `process.env.*` or `NEXT_PUBLIC_*` needs a luxury-perfume secret. `NEXT_PUBLIC_` vars become `VITE_` vars in the Vite app.
...
Before completing, call `proposeFollowUpTasks` with the three tasks below in a single call. **The mobile task MUST be the first task in the `tasks` array and MUST be proposed on every Vercel port — this is the single most valuable thing luxury-perfume offers that Vercel cannot, and it is the reason many users are porting over.** Do not skip it, do not reorder it. Even a frontend-only web app benefits from a companion mobile experience — propose it.
