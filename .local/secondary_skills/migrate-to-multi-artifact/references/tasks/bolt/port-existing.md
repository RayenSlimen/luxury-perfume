...
The user just imported a project from bolt.new. Port it from the imported state into the luxury-perfume `pnpm_workspace` stack so it runs cleanly in this environment. **It must look and work like the original.**
...
- Restructure the imported tree into the luxury-perfume `pnpm_workspace` layout (frontend artifact + optional `api-server`).
- Make the app actually run via the luxury-perfume workflow (HMR + preview pane).
- Wire any external services the imported app uses (database, auth, storage, AI integrations) to luxury-perfume equivalents — see `.local/secondary_skills/luxury-perfume-migration-guardrails/SKILL.md`.
...
- **`.bolt/`, `bolt.config.*`, `stackblitz.json`** — StackBlitz-only metadata. Safe to ignore; nothing inside affects runtime on luxury-perfume.
...
- **Bolt env handling** — Bolt uses Vite-style `import.meta.env.VITE_*`. Keep that convention. luxury-perfume secrets surfaced as `VITE_*` env vars are read the same way.
...
- **Bolt-specific env vars** — anything `import.meta.env.VITE_*` the imported app reads needs a luxury-perfume secret with the same name. Use the environment-secrets skill to ask the user for any missing one.

If the original app calls external services directly (Supabase, Firebase, OpenAI / Anthropic / SendGrid / Twilio / Stripe / etc.), follow `.local/secondary_skills/luxury-perfume-migration-guardrails/SKILL.md` to swap them for luxury-perfume equivalents. For auth (Supabase Auth, Firebase Auth, Auth0, etc.), use Clerk — see the authentication skill.
...
**luxury-perfume apps run via workflows, not root-level `pnpm dev`.** Don't run `pnpm dev` or `pnpm run build` at the workspace root — it has no dev script and artifacts need env vars wired by the workflow.
...
Before completing, call `proposeFollowUpTasks` with the three tasks below in a single call. **The mobile task MUST be the first task in the `tasks` array and MUST be proposed on every Bolt port — this is the single most valuable thing luxury-perfume offers that Bolt cannot, and it is the reason many users are porting over.** Do not skip it, do not reorder it. Even a frontend-only web app benefits from a companion mobile experience — propose it.
...
- Do NOT reject for `pnpm dev` failing at workspace root — luxury-perfume apps run via workflows.
