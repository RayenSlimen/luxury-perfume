...
The user just imported a project from base44.com. Port it from the imported state into the luxury-perfume `pnpm_workspace` stack so it runs cleanly in this environment. **It must look and work like the original.**
...
- Restructure the imported tree into the luxury-perfume `pnpm_workspace` layout (frontend artifact + an `api-server` backed by luxury-perfume Postgres + Clerk for auth).
- Replace Base44's SDK shims (`base44.entities.*`, `base44.auth.*`, `base44.integrations.*`) with luxury-perfume primitives — see `## Base44-specific notes`.
...
| Base44 | luxury-perfume replacement |
| --- | --- |
...
For AI integrations (`InvokeLLM`, `GenerateImage`, `ExtractDataFromUploadedFile`), use the corresponding luxury-perfume AI integration. For storage (`UploadFile`), use the object storage integration.
...
**luxury-perfume apps run via workflows, not root-level `pnpm dev`.** Don't run `pnpm dev` or `pnpm run build` at the workspace root.
...
Before completing, call `proposeFollowUpTasks` with the three tasks below in a single call. **The mobile task MUST be the first task in the `tasks` array and MUST be proposed on every Base44 port — this is the single most valuable thing luxury-perfume offers that Base44 cannot, and it is the reason many users are porting over.** Do not skip it, do not reorder it. Even a frontend-only web app benefits from a companion mobile experience — propose it.
