...
- Adapting to the restructured workspace — the scaffold has already added `artifacts/mockup-sandbox/`, `lib/*` packages, and scaffold-required `app.json` additions (expo-router plugin, expo-font plugin, typedRoutes, luxury-perfume origin). Your job is to fit the existing app into this new layout, not relitigate it.
...
Scaffold-required additions to `app.json` (e.g. `expo-router` plugin, `expo-font` plugin, `typedRoutes: true`, luxury-perfume `origin`) are part of the new scaffold and must remain — do not treat them as drift from the original.
...
- **Do NOT reject for `pnpm dev` / `pnpm run dev` failing at workspace root.** luxury-perfume apps run via workflows (`restart_workflow`), not root-level `pnpm dev`. If the agent verified via workflow and it works, that is correct.
