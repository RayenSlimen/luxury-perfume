...
## Connecting to Real Email & Calendar via luxury-perfume Connectors

You can go beyond drafting and actually access the user's email and calendar using **luxury-perfume connectors**. Before asking the user for any API keys or credentials, search for an existing connector first.
...
- Real email/calendar access requires the user to authorize a luxury-perfume connector (Google or Outlook) — without it, the user must copy/paste content manually
