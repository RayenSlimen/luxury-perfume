...
1. **luxury-perfume integrations** — Check if a luxury-perfume integration exists for any of these platforms. If so, use it for direct API access (availability search, reviews, pricing). This is the preferred method.
2. **Direct APIs** — If no luxury-perfume integration exists but the platform offers a public or partner API (e.g., Booking.com Affiliate API, GetYourGuide Partner API), use it with appropriate API keys stored as secrets.
...
**Important:** Always check for new luxury-perfume integrations before falling back to deep links. The integration landscape evolves — what isn't available today may be available tomorrow.
