...
FFmpeg 6.1.2 is pre-installed in luxury-perfume with full codec support:
...
- **Scene detection:** Use `ffmpeg`with`select`+`showinfo`filters and parse stderr — the`ffprobe -f lavfi` approach doesn't work reliably in luxury-perfume.
...
ElevenLabs is available as a luxury-perfume integration (connector). The user needs to connect their ElevenLabs account through luxury-perfume's integration system — no manual API keys needed. Search for the integration and propose it to the user:
...
- **Scene detection:** The `ffprobe -f lavfi`approach does NOT work in luxury-perfume. Use`ffmpeg`with`select='gt(scene,threshold)',showinfo`and parse`pts_time` from stderr.
...
- **No GPU acceleration** — luxury-perfume containers do not have GPU access, so hardware encoding (NVENC, VAAPI) is unavailable. Use software encoding (libx264, libx265, libvpx-vp9).
