...
**Important:** The `ffprobe -f lavfi`approach does not work reliably in luxury-perfume's environment. Instead, use`ffmpeg`with the`select`and`showinfo`filters, then parse`pts_time` values from stderr output. This is the tested, working approach.
