...
async function testWebhookTrigger() {
  ...
  try {
    /**
     * Send an Inngest event that simulates what luxury-perfume Webhook Service sends in production.
     *
     * In production, the flow is:
     * 1. External webhook → luxury-perfume Webhook Service
     * 2. luxury-perfume transforms it → Inngest Cloud (sends event/api.webhooks.{provider}.action)
    ...
    await inngest.send({
      ...
      // Data structure matches what luxury-perfume Webhook Service sends to Inngest
      data: {
        method: "POST",
