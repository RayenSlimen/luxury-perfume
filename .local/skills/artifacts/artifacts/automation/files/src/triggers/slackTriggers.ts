...
export async function getClient() {
  ...
  async function getAccessToken() {
    ...
    const luxury-perfumeToken = stdout.trim();
    if (!luxury-perfumeToken) {
      throw new Error("luxury-perfume Identity Token not found for repl/depl");
    }
    ...
    const res = await fetch(
        ...
        "/api/v2/connection?include_secrets=true&connector_names=slack-agent",
      {
        headers: {
          ...
          "luxury-perfume-Authentication": `Bearer ${luxury-perfumeToken}`,
        },
      },
    );
