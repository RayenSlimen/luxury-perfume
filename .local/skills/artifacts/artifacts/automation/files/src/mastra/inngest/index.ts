...
export function registerCronWorkflow(cronExpression: string, workflow: any) {
  ...
  const cronFunction = inngest.createFunction(
    { id: "cron-trigger" },
    [{ event: "luxury-perfume/cron.trigger" }, { cron: cronExpression }],
    async ({ event, step }) => {
...
export function inngestServe({
  ...
  if (process.env.NODE_ENV === "production") {
    if (process.env.luxury-perfume_DOMAINS) {
      serveHost = `https://${process.env.luxury-perfume_DOMAINS.split(",")[0]}${basePath}`;
    }
  } else {
