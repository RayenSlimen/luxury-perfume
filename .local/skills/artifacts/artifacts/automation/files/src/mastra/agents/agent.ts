 ...
 *
 * IMPORTANT: Both approaches require the SAME syntax for luxury-perfume Playground compatibility:
 * - Use AI SDK v5: model, e.g. openai("gpt-4o-mini")
 * - In workflows: Use agent.generate()
 * - The luxury-perfume Playground UI always calls the legacy Mastra endpoint
 * NOTE: You must always keep the API key as an environment variable for safety!
 * ---
 * OPTION 1: luxury-perfume AI Integrations, **only** if user has enabled it via connector.
 *
 * No OpenAI API key required - charges billed to luxury-perfume credits
...
`,
   ...
   *
   * MUST use AI SDK v5 syntax for luxury-perfume Playground compatibility.
