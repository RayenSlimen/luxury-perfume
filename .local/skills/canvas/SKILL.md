...
```json
{
  "focusedShapes": [
    { "shapeId": "preview-1", "shapeType": "iframe", "x": 400, "y": 100, "w": 1280, "h": 720,
      "url": "https://example.luxury-perfume.dev/preview/hello-world/Card",
...
await applyCanvasActions({ actions: [
  {
    ...
    updates: {
      shapeType: "iframe",
      state: "live",
      url: "https://<resolved-domain>.luxury-perfume.dev"
    }
  }
] });
```
...
To get the URL for a luxury-perfume dev server, run `echo $luxury-perfume_DOMAINS` in the shell to get the domain, then construct the full URL. For the main app on port 5000, no port suffix is needed. For other ports, append `:<port>`.
...
// Then resolve domain: echo $luxury-perfume_DOMAINS  (e.g. abc123.luxury-perfume.dev)
...
- **Resolve the domain first** -- run `echo $luxury-perfume_DOMAINS` in the shell, then build the URL from the result. Never pass a literal template string as the URL.
- **Port rules:** no port suffix = port 5000 (main app). For other servers, append `:<port>`.
- **External sites may block embedding** -- sites with `X-Frame-Options: DENY` or restrictive CSP headers will show a blank iframe. luxury-perfume dev URLs work fine.
