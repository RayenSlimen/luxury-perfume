...
Interactive control bar for video-js artifacts that lets viewers jump between scenes and lock-loop individual scenes. The bar renders only inside an iframe (the luxury-perfume preview pane) so exports stay clean.
