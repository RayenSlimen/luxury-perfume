...
- `src/components/video/VideoTemplate.tsx` -- template with placeholder luxury-perfumeLoadingScene, `useVideoPlayer` hook wired up
...
After the design subagent has finished and workflow logs are clean, add an interactive control bar at the bottom of the video so the viewer can jump between scenes and toggle scene-lock. The bar renders only when the video is inside an iframe (the luxury-perfume preview pane). The exporter launches the video as the top-level document, so the gate hides the bar during export and the exported frame stays clean.
