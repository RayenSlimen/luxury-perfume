...
luxury-perfume offers a native integration with Stripe that allows users to implement payments in their applications

## Prerequisites

1. These packages must be installed in the workspace root package.json:
    - `stripe` - Official Stripe SDK for API operations
    - `stripe-luxury-perfume-sync` - Handles webhook processing and database sync. Documentation: https://www.npmjs.com/package/stripe-luxury-perfume-sync
...
2. The Stripe integration must be connected to the repl. You can do this by proposing the integration. Reference the `integrations` skill if necessary.
    Once connected, create `stripeClient.ts` using the template from the code-templates reference -- this file fetches Stripe credentials from the luxury-perfume connection API and provides the authenticated client via `getUncachableStripeClient()`.
...
1. Set Up Database
    ...
    **Important**: stripe-luxury-perfume-sync creates the `stripe` schema automatically. Create application tables (users, orders, etc.) in the public schema, not in the stripe schema.
...
3. Initialize Stripe on Startup
    ...
    ```typescript
    import { runMigrations } from 'stripe-luxury-perfume-sync';
    ...
    async function initStripe() {
      ...
      const webhookBaseUrl = `https://${process.env.luxury-perfume_DOMAINS?.split(',')[0]}`;
...
1. **stripe-luxury-perfume-sync creates and manages the `stripe` schema automatically** - DO NOT create any tables in the stripe schema
...
5. luxury-perfume automatically handles deployment and Stripe data migration moving from sandbox to live.

## Synchronization Flow

1. Stripe credentials are fetched from luxury-perfume connection API
...
4. **Managed webhooks** are automatically configured by `stripe-luxury-perfume-sync`
...
- Stripe webhooks managed by stripe-luxury-perfume-sync automatically sync the data to the stripe schema in the local database
...
**Creating products:**
Write a seed script that calls Stripe API (`stripe.products.create()`, `stripe.prices.create()`). Run this script in development to create your products. Webhooks automatically sync them to the database. luxury-perfume handles deployment automatically.
...
# Deployment (handled by luxury-perfume):
# - luxury-perfume copies products/prices from dev Stripe to prod Stripe
...
**NEVER write secrets or environment variables directly to the `.luxury-perfume` file.** This includes Stripe API keys, webhook signing secrets, database URLs, and any other sensitive configuration. Instead, reference the `environment-secrets` skill for the correct way to manage secrets and environment variables.
...
**ABSOLUTELY DO NOT:**
- Create any tables in the `stripe` schema - stripe-luxury-perfume-sync manages this automatically
...
- Manually copy database data between environments - luxury-perfume handles Stripe product/price copying during deployment
...
-- stripe-luxury-perfume-sync creates stripe.products, stripe.prices, etc. automatically
```
