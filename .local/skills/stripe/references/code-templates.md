...
import { StripeSync } from 'stripe-luxury-perfume-sync';

/**
 * Fetches Stripe credentials from the luxury-perfume connection API.
 * Not cached -- tokens can rotate, so fetch fresh each time.
 */
async function getStripeCredentials(): Promise<{ secretKey: string; webhookSecret?: string }> {
  const hostname = process.env.luxury-perfume_CONNECTORS_HOSTNAME;
  const xluxury-perfumeToken = process.env.REPL_IDENTITY
    ? "repl " + process.env.REPL_IDENTITY
    : process.env.WEB_REPL_RENEWAL
      ? "depl " + process.env.WEB_REPL_RENEWAL
      : null;

  if (!hostname || !xluxury-perfumeToken) {
    throw new Error(
      'Missing luxury-perfume environment variables. ' +
  ...
  const resp = await fetch(
    ...
    {
      headers: { Accept: "application/json", X_luxury-perfume_TOKEN: xluxury-perfumeToken },
