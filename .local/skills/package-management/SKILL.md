...
## Modules (luxury-perfume Terminology)

"Modules" is a luxury-perfume-specific term for language toolchains that can be installed into the NixOS environment. Use `listAvailableModules()` to see what's available.
...
- Never add luxury-perfume config files to `.gitignore`
