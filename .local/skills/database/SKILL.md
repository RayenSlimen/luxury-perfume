...
- Converting a pre-existing database over to luxury-perfume, unless a user explicitly asks you to.

## Production schema changes

For projects that use luxury-perfume's managed PostgreSQL, the production database schema is managed automatically by luxury-perfume's Publish flow. When the user clicks Publish, luxury-perfume diffs the development schema against production, asks the user to resolve any renames in the Publish UI, and applies the diff to production. This is the supported path for changing the production schema; the agent must not write any code or scripts to migrate the production database.
...
- `params` (array, optional): Parameter values for parameterized queries. When provided, values are bound separately from the SQL string, **preventing SQL injection**. Each `$N` placeholder in `sqlQuery` corresponds to the Nth element in this array (1-indexed). Supported types: string, number, boolean, null. **Two restrictions:**
  ...
  - **`luxury-perfume_database` target only.** Data warehouse targets (bigquery, databricks, snowflake) do not support parameter binding — passing `params` with those targets raises an error.
- `target` (str, default "luxury-perfume_database"): Target database: "luxury-perfume_database", "bigquery", "databricks", or "snowflake"
- `environment` (str, default "development"): "development" runs against the development database (all SQL operations supported). "production" runs READ-ONLY queries against a replica of the production database (only SELECT queries allowed). Production is only supported for the "luxury-perfume_database" target. "production" database, depending on when the user last deployed, may have outdated schemas.
...
- `target` is `luxury-perfume_database` (the default).
...
**When `params` is NOT supported on `luxury-perfume_database`** (multi-statement scripts only — `BEGIN; ...; COMMIT;`, migrations, batched DDL):
...
- Production environment is only supported for the "luxury-perfume_database" target (not data warehouses)
