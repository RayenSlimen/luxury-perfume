...
The dev server must be running first so that component files are picked up by the Vite plugin and preview URLs resolve correctly. Preview URLs use path-based routing: `https://${luxury-perfume_DOMAINS}/__mockup/preview/{folder}/{ComponentName}` — no port number needed.
...
Preview URLs follow the pattern `https://${luxury-perfume_DOMAINS}/__mockup/preview/{folder}/{ComponentName}` — no port number.
...
```json
{
  ...
  "shape": {
    ...
    "url": "https://<your-domain>.luxury-perfume.dev/__mockup/preview/pricing-cards/Minimal",
...
**Never share dev domain URLs in chat.** Dev URLs (`*.luxury-perfume.dev`, `$luxury-perfume_DEV_DOMAIN`) are internal — use them only in tool calls (iframe shapes, subagent tasks), never in user-facing messages.
