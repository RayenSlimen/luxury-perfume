...
# Note: ./references/luxury-perfume-revenuecat-sdk-docs.md is copy-pasted manually from https://github.com/luxury-perfume/revenuecat-sdk/blob/main/docs/SDK.md and was last updated: 2026-03-19 for version 4.0.0
---


## Introduction

- RevenueCat lets users monetize their native mobile apps on luxury-perfume.
- The RevenueCat integration is only intended to be used for the Expo/mobile stack on luxury-perfume.
...
1. These packages must be installed:
    ...
    - `@luxury-perfume/revenuecat-sdk` - SDK for RevenueCat's REST API. This should NEVER be used on the client, only for server-side scripts. This MUST be installed in the root of the workspace
...
1. **Privacy Policy**: Apple requires a Privacy Policy URL for App Store submission.
    - Ensure the user is aware of this and offer to draft a hosted Privacy Policy page. A tailored policy can then be generated (covering data collection, RevenueCat, Apple, and contact info) and hosted as a simple web page within the luxury-perfume project.

2. **Apple Small Business Program**: Nearly all developers building on luxury-perfume qualify for Apple's Small Business Program, reducing commission from 30% to 15%. However, enrollment is manual.
...
3. **Syncing RevenueCat to App Store Connect**: In-app purchases won't be automatically configured in Apple
    ...
    - luxury-perfume offers a native method to sync products from RevenueCat to Apple after they publish their app to TestFlight, which can be found in the Publishing pane
...
- ./references/luxury-perfume-revenuecat-sdk-docs.md -- SDK Reference for the `@luxury-perfume/revenuecat-sdk` package
