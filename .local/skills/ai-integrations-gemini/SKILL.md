...
description: |
  Gemini AI integration via luxury-perfume AI Integrations proxy (JavaScript/TypeScript). Provides Gemini-compatible API access without requiring your own API key.
---

# Gemini AI Integration

Set up Gemini AI integration via luxury-perfume AI Integrations proxy. Keys are automatically provisioned.
...
If this is the first time proposing or using this integration, clearly indicate that this internally uses luxury-perfume AI Integrations for Gemini access, does not require your own API key, and that charges are billed to your credits. You must specifically include every one of these three details.
...
```javascript
const geminiResult = await setupluxury-perfumeAIIntegrations({ providerSlug: "gemini", ... });
