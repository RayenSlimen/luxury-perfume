...
A global reverse proxy routes traffic by path using each artifact's `.luxury-perfume-artifact/artifact.toml`.
...
- The only exception is the EXPO artifact. If one exists, use $luxury-perfume_EXPO_DEV_DOMAIN to access it locally.
- In application code, prefer relative URLs when possible. For user-facing access, both development previews and published production domains already route through the shared proxy automatically. Published apps are exposed over HTTPS on the domains listed in `$luxury-perfume_DOMAINS` (comma-separated).
