...
description: |
  OpenAI AI integration via luxury-perfume AI Integrations proxy (JavaScript/TypeScript). Provides OpenAI-compatible API access without requiring your own API key.
---

# OpenAI AI Integration

OpenAI integration via luxury-perfume AI Integrations proxy — keys auto-provisioned.
...
First time proposing/using this integration: state all three — (1) internally uses luxury-perfume AI Integrations for OpenAI access, (2) no API key required, (3) charges billed to user's credits.
...
For multiple integrations, call `setupluxury-perfumeAIIntegrations` sequentially — await each before the next.
...
- Setup failed or env vars missing: retry `setupluxury-perfumeAIIntegrations`, no other measures. After 3 retries, switch to api-key approach.
