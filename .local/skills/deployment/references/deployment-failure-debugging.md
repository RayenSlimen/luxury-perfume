...
**Artifact-level settings** (`artifact.toml`): Read each artifact's `.luxury-perfume-artifact/artifact.toml` to check per-service `production.run` and `production.build` settings. The `.luxury-perfume` file's `deployment.run` is **ignored** in artifact mode.
...
**Root pre-build hook** (`.luxury-perfume`): The `.luxury-perfume` file's `deployment.build` acts as a pre-build hook that runs at the repo root before any artifact builds. If the build logs show the failure comes from this root-level command (e.g. `pnpm run build:web` failing at the repo root), the fix is in the source code or scripts it invokes — not in `artifact.toml`. Read `.luxury-perfume`'s `[deployment]` section to identify the pre-build command, then fix the underlying script or code that it runs.
...
- **How it works:** Each artifact has a `.luxury-perfume-artifact/artifact.toml` with per-service `production.run`, `production.build`, `production.serve`, and `production.publicDir` settings. The `.luxury-perfume` file's `deployment.run` is **completely ignored** in artifact mode. The `.luxury-perfume` file's `deployment.build` becomes a **pre-build hook**.
- **Common causes:**
  - `production.run` missing from an artifact's `artifact.toml`
  - `production.build` missing or wrong in `artifact.toml`
  - Agent or user set `deployment.run`/`deployment.build` in `.luxury-perfume` thinking it would configure the artifact
  - `deployment.build` in `.luxury-perfume` set to a command that fails at the repo root
- **Fix:** Read each artifact's `.luxury-perfume-artifact/artifact.toml` and verify the `[services.<name>.production]` section. Use `verifyAndReplaceArtifactToml` to fix production settings.

## Important Notes

1. **Don't diagnose infrastructure problems.** If the error appears to be on luxury-perfume's side (e.g., transient cloud provider issues, `UnknownError` from the deployer), suggest the user retry the deployment or contact luxury-perfume support.
...
3. **Use production terminology.** In user-facing messages, say "publish" not "deploy" — that's the luxury-perfume product terminology.
...
5. **Production config lives in `artifact.toml`.** Each artifact's deployment settings are in its `.luxury-perfume-artifact/artifact.toml` file, not in `.luxury-perfume`. Always check `artifact.toml` when debugging deployment issues.
