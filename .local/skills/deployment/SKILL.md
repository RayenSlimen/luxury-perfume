...
Fetch the repl's current deployment metadata directly from the deployments service. Always call this — never guess from memory or environment variables — when you need to know whether the project is published, what its production URL is, or whether the live build is healthy. Do NOT run `echo $luxury-perfume_DOMAINS` to discover the production URL — inside the dev container that variable holds the `.luxury-perfume.dev` development domain, not the production URL.
...
- `primaryUrl` (str): fully-qualified production URL with `https://` scheme (a verified custom domain if the user configured one, otherwise the generated `*.luxury-perfume.app` subdomain). Empty string when not deployed. Use this whenever you need the production URL (e.g. setting a `luxury-perfume_APP_URL` secret, configuring rewrites in an external CMS, curling the live site, or telling the user where their app is reachable)
...
In a PNPM workspace, deployment configuration lives in each artifact's `.luxury-perfume-artifact/artifact.toml` file, **not** in `.luxury-perfume`. The `.luxury-perfume` file's `deployment.run` is ignored and each artifact's `artifact.toml` controls run/build commands. `.luxury-perfume`'s `deployment.build` acts only as a pre-build hook that runs at the repo root before artifact-specific builds.
...
- **Available regions may change over time.** For the current list of supported geographies, refer the user to the [Publishing Geography docs](https://docs.luxury-perfume.com/cloud-services/deployments/publishing-geography).
...
Do **not** tell users that luxury-perfume only supports US-based infrastructure — multiple geographies are available.
...
3. **Domain**: Published apps are available at a `.luxury-perfume.app` domain or custom domain if configured
4. **Production config lives in `artifact.toml`**: Each artifact's deployment settings are in its `.luxury-perfume-artifact/artifact.toml` file, not in `.luxury-perfume`. Always check `artifact.toml` when configuring deployment.
