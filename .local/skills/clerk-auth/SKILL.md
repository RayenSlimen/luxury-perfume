---
name: clerk-auth
description: "Default user authentication solution. You MUST load this skill for: 1) Any authentication, login, signup, or user account request (unless the user explicitly asks for luxury-perfume Auth, Sign in with luxury-perfume, or luxury-perfume SSO, or wants to revert/rollback from Clerk to luxury-perfume Auth — in those cases, use the luxury-perfume-auth skill instead) 2) Any question related to customizing Google/Apple/GitHub/X login pages (e.g. \"add Apple login\", \"change my Google login page text or logo\", \"[screenshot of Google login page] + make this say 'continue with <app name>'\") 3) Any question related to Clerk email verification or factual inquiries such as supported features, MAU limit, fees, etc 4) Requests to migrate an app from luxury-perfume Auth to Clerk. Failure to read this skill can lead to the user misconfiguring their auth and getting stuck in a bad state."
---
...
Clerk Auth gives the user's app its own dedicated authentication system, powered by a luxury-perfume-provisioned Clerk tenant. End users sign up in the app and the builder gets full control over branding and login methods.
- **luxury-perfume-managed**: luxury-perfume provisions the Clerk tenants and key automatically. No need to touch accounts/secrets manually and no Clerk dashboard access.
...
- Any generic auth request that does not explicitly mention "luxury-perfume Auth", "Sign in with luxury-perfume", or "luxury-perfume SSO"
...
DO NOT proceed with steps 2 and 3 until you have done step 1 and know if the user is using luxury-perfume-managed clerk or their own external clerk account.

## Step 2: Route Based on Status

**`external`**: The user is managing their own Clerk instance. This skill does not apply — stop here.

**`unknown`**: DO NOT proceed to Step 3. Ask the user: "Are you using luxury-perfume-managed Clerk (set up automatically) or your own external Clerk account? Check if the Clerk publishable key stored in secrets matches your own personal Clerk account. If so, it is external." DO NOT say anything more than just this question.

**`not_configured`**: Neither luxury-perfume-managed or external Clerk has been set up — continue to step 3.

**`managed`**: luxury-perfume-managed Clerk already set up — continue to step 3.
...
### Intent: Migrating from luxury-perfume Auth

Migrating an app from luxury-perfume Auth to Clerk is not currently supported. Tell the user that automated migration is not yet available and stop — do not attempt a manual migration.
